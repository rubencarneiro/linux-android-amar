...
{
	if (pgc->plcm->drv->set_cabc_mode) {
		pgc->plcm->drv->set_cabc_mode(cabc_state);
	}
	return;
}
	...
	struct device_attribute *attr,const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "1", 1)) {
		strcpy(cabc, "1");
		mtkfb_set_cabc(CABC_enable);
	} else if (!strncmp(buf, "0", 1)) {
		strcpy(cabc, "0");
		mtkfb_set_cabc(CABC_disable);
	}

	return count;
}

static DEVICE_ATTR(cabc, S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP, cabc_show, cabc_store);
static int mtk_fb_create_param_sysfs(void)
{
	int rc = 0;

	rc = device_create_file(mtkfb_fbi->dev, &dev_attr_cabc);
	if (rc) {
		pr_err("failed to create sysfs for %d\n", rc);
	}
	return rc;
}
...
char *mtkfb_find_lcm_driver(void)
{
	...
	lcm_detect_name = mtkfb_lcm_name;
	printk("func:%s  zhengyang   %s  %s\n", __func__, __LINE__, mtkfb_lcm_name);
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	r = mtk_fb_create_param_sysfs();
	if (r) {
		DISPERR("panel parameter sysfs creation failed, r = %d\n", r);
	}
