...
extern void camaf_power_init(void);
extern void camaf_power_on(void);
extern void camaf_power_off(void);
	...
	enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	if (strcmp(curr_sensor_name,"s5k4h7yx_mipi_raw")==0 || strcmp(curr_sensor_name,"s5k4h7yxqtech_mipi_raw")==0)
	{
		pr_info("AFRegulatorCtrl sensor_idx %d, power %d curr_sensor_name %s\n", sensor_idx, pwr_status, curr_sensor_name);
