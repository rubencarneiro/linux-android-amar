...
#include "s5k4h7yxqtechmipiraw_Sensor.h"

#define PFX "s5k4h7yxqtech_camera_sensor"
...
#define DEVICE_VERSION_S5K4H7YXQTECH    "s5k4h7yxqtech"
...
extern unsigned char s5k4h7yxqtech_get_module_id(void);
#endif
//extern unsigned main_sub_flag;
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7YXQTECH_SENSOR_ID,  //S5K4H7YXQTECH_SENSOR_ID = 0x487C
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
 { 3264, 2448, 0,   0,   3264, 2448, 3264, 2448, 0000, 0000, 3264, 2448,	0,	0, 1632, 1224}, // Preview
...
static void sensor_reg_dump(void)
{
	...
	LOG_ERR("+");
	LOG_ERR("Camera_S5K4H7YXQTECH_sensor RegDump start");

	LOG_ERR("frame length[5:0]-[13:8] -- default: 0x09 --  now: 0x%x\n", read_cmos_sensor_8(0x0340));
	LOG_ERR("frame length[7:0]-[ 7:0] -- default: 0xE2 --  now: 0x%x\n", read_cmos_sensor_8(0x0341));
	LOG_ERR("frame length = 0x%x (default: 0x09E2)\n", (read_cmos_sensor_8(0x0340) << 8) | read_cmos_sensor_8(0x0341));
	...
	LOG_ERR("fine_integration_time -- default: 0x0DD8 --  now: 0x%x\n", fine_integration_time);
	LOG_ERR("coarse_integration_time -- default: 0x04F0 --  now: 0x%x\n", coarse_integration_time);
...
static void write_shutter(kal_uint32 shutter)
	{
		...
		if(imgsensor.sensor_mode == IMGSENSOR_MODE_HIGH_SPEED_VIDEO)
		{
			...
			LOG_ERR("shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
			return;
		}
		spin_lock(&imgsensor_drv_lock);
		if (shutter > imgsensor.min_frame_length - imgsensor_info.margin)
...
static kal_uint16 s5k4h7_table_write_cmos_sensor(
...
static kal_uint16 addr_data_pair_init_s5k4h7[] = {
	0x0100, 0x00,
	0x0B05, 0x01,
	0x3074, 0x06,
	0x3075, 0x2F,
	...
	0x3C32, 0xFF,
	0x0A02, 0x14,
	0x3931, 0x02,//VOD 470mV
	0x3932, 0x80,//LP up
	0x3930, 0x80,//tr/tf 2step up
	0x392F, 0x01,//tr/tf 2step up
};
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	LOG_ERR("streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable) {
		...
		if ( framecnt != 0xFF) {
			LOG_ERR(" Stream ON OK framecnt = %d.\n", framecnt);
		}
	} else {
		//write_cmos_sensor(0x6028,0x4000);
...
static void sensor_init(void)
{
	...
	s5k4h7_table_write_cmos_sensor(addr_data_pair_init_s5k4h7,
	   sizeof(addr_data_pair_init_s5k4h7)/sizeof(kal_uint16));
}	/*	sensor_init  */

//MTK 2x2Bin 1632x1224  30.01fps
static kal_uint16 addr_data_pair_preview_s5k4h7[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static void preview_setting(void)
{
	...
	s5k4h7_table_write_cmos_sensor(addr_data_pair_preview_s5k4h7,
	   sizeof(addr_data_pair_preview_s5k4h7)/sizeof(kal_uint16));
}	/*	preview_setting  */

//MTK 3264x2448  15fps
static kal_uint16 addr_data_pair_capture_s5k4h7_15fps[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static kal_uint16 addr_data_pair_capture_s5k4h7_24fps[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static kal_uint16 addr_data_pair_capture_s5k4h7[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...

{
	...
	if (currefps == 150) {
		...
		/*Delay 1 frame*/
		//mDELAY(67);
		s5k4h7_table_write_cmos_sensor(
			addr_data_pair_capture_s5k4h7_15fps,
	sizeof(addr_data_pair_capture_s5k4h7_15fps)/sizeof(kal_uint16));
...
/*mipi_datarate:836,pvi_pclk_inverwrite_cmos_sensor*/
		...
		//mDELAY(42);
		s5k4h7_table_write_cmos_sensor(
			addr_data_pair_capture_s5k4h7_24fps,
	sizeof(addr_data_pair_capture_s5k4h7_24fps)/sizeof(kal_uint16));
	} else{
		/*Delay 1 frame*/
		//mDELAY(33);
		s5k4h7_table_write_cmos_sensor(
			addr_data_pair_capture_s5k4h7,
	sizeof(addr_data_pair_capture_s5k4h7)/sizeof(kal_uint16));
	}
}

#if 0
static kal_uint16 addr_data_pair_video_s5k4h7[] = {
	0x0100, 0x00,
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	//mDELAY(33);
#if 0
	s5k4h7_table_write_cmos_sensor(addr_data_pair_video_s5k4h7,
		sizeof(addr_data_pair_video_s5k4h7)/sizeof(kal_uint16));
#endif
	s5k4h7_table_write_cmos_sensor(addr_data_pair_capture_s5k4h7,
		sizeof(addr_data_pair_capture_s5k4h7)/sizeof(kal_uint16));

}

//MTK 4x4Bin 640x480  120.13fps
static kal_uint16 addr_data_pair_hs_video_s5k4h7[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static void hs_video_setting(void)
{
	...
	/*Delay 1 frame*/
	//mDELAY(8);
	s5k4h7_table_write_cmos_sensor(addr_data_pair_hs_video_s5k4h7,
		sizeof(addr_data_pair_hs_video_s5k4h7)/sizeof(kal_uint16));
}

//MTK 2x2Bin 1280x720  30.01fps
static kal_uint16 addr_data_pair_slim_video_s5k4h7[] = {
	0x0136, 0x18,
	0x0137, 0x00,
	0x0305, 0x06,
	0x0306, 0x00,
...
static void slim_video_setting(void)
{
	...
	/*Delay 1 frame*/
	//mDELAY(33);
	s5k4h7_table_write_cmos_sensor(addr_data_pair_slim_video_s5k4h7,
		sizeof(addr_data_pair_slim_video_s5k4h7)/sizeof(kal_uint16));
}


#define OTP_4H7Q 1
#define INCLUDE_NO_OTP_4H7Q 0
#if OTP_4H7Q
#define S5K4H7YXQTECH_EEPROM_SLAVE_ADD 0xB0
#define S5K4H7YXQTECH_SENSOR_IIC_SLAVE_ADD 0x20
#define S5K4H7YXQTECH_MODULE_ID  0x06

typedef struct s5k4h7yxqtech_otp_data {
	unsigned short module_id;
} S5K4H7YXQTECH_OTP_DATA;

S5K4H7YXQTECH_OTP_DATA s5k4h7yxqtech_otp_data;
...
static int  s5k4h7yxqtech_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add,kal_uint8 size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		...
		LOG_INF("+++4H7 otp module_id[%d] = 0x%x\n",i,module_id[i]);
		start_add ++;
	}
	s5k4h7yxqtech_otp_data.module_id = module_id[0];
	LOG_ERR("s5k4h7yxqtech_otp_data.module_id= 0x%x",s5k4h7yxqtech_otp_data.module_id);

	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = S5K4H7YXQTECH_SENSOR_IIC_SLAVE_ADD;
	spin_unlock(&imgsensor_drv_lock);

	return s5k4h7yxqtech_otp_data.module_id;
}
#endif
/*************************************************************************
* FUNCTION
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001)) + 2;
			...
			// imgsensor_info.module_id = s5k4h7yxqtech_get_module_id();
...
#if OTP_4H7Q
				s5k4h7yxqtech_read_data_from_eeprom(S5K4H7YXQTECH_EEPROM_SLAVE_ADD,0x000B,1);
#if INCLUDE_NO_OTP_4H7Q
				if ((s5k4h7yxqtech_otp_data.module_id > 0) && (s5k4h7yxqtech_otp_data.module_id < 0xFFFF)) {
#endif
					if (s5k4h7yxqtech_otp_data.module_id != S5K4H7YXQTECH_MODULE_ID) {
						*sensor_id = 0xFFFFFFFF;
						return ERROR_SENSOR_CONNECT_FAIL;
					} else {
						LOG_INF("This is qtech --->s5k4h7yx otp data vaild ...");
						hq_regiser_hw_info(HWID_MAIN_CAM, "qtech s5k4h7yx 8m");
					}
#if INCLUDE_NO_OTP_4H7Q
				} else {
					LOG_INF("This is s5k4h7yx, but no otp data ...");
				}
#endif
#endif
				return ERROR_NONE;
		...
		do {
			...
			if (sensor_id == imgsensor_info.sensor_id) {
				...
				if ((s5k4h7yxqtech_otp_data.module_id > 0) && (s5k4h7yxqtech_otp_data.module_id < 0xFFFF)) {
					...
					LOG_INF("This is s5k4h7yxqtech, but no otp data ...");
				}
#endif
#endif
				break;
...
UINT32 S5K4H7YXQTECH_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	s5k4h7yxqtech_MIPI_RAW_SensorInit	*/
