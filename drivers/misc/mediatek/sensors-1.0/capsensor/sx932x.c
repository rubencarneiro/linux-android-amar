...
{    
	...
	if (this){
		...
		* (1->NIRQ=0, 0->NIRQ=1) */
		...
		err = read_register(this, SX932x_WHOAMI_REG, &readcode);
		if(err < 0){
			printk("Sx932x.km probe Fail!\n");
			return err;
		}
	...
	hq_regiser_hw_info(HWID_CAPSENSOR, "IC:sx932x");
...
{
	psx93XX_t this = dev_get_drvdata(dev);
	printk(KERN_ERR "sx932x resume begin\n");
	sx93XX_resume(this);
	printk(KERN_ERR "sx932x resume done\n");
	return 0;
}
/*====================================================*/
#else
