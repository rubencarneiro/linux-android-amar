...
static ssize_t psenable_store(struct device *dev, struct device_attribute *attr,
                                  const char *buf, size_t count)
{
	struct alsps_context *cxt = NULL;
	int value = -1;
	...
	printk("wanwen enable is %s\n", buf);
	err = kstrtoint(buf, 10, &enable);
	if (err != 0)
		pr_err("ps_store_enable() param error: err = %d\n", err);

	printk("wanwen enable is %d\n",enable);
	if (enable == 25) {
	        psensor_25_offset = 0;
			cxt = alsps_context_obj;
			for (i=0;i<11;i++) {
			if (cxt->ps_data.ps_get_raw_data != NULL) {
				err = cxt->ps_data.ps_get_raw_data(&value);
			}
			if (i==0) {
                value=0;
			}
			...
			psensor_25_offset = value+psensor_25_offset;
			pr_err("wanwen get alsps data value is %d\n",psensor_25_offset);
		}
		pr_err("wanwen1 get alsps data value is %d\n",psensor_25_offset);
		psensor_25_offset = psensor_25_offset/10;
		pr_err("wanwen2 get alsps data value is %d\n",psensor_25_offset);
			...
			psensor_ct = value+psensor_ct;
			pr_err("wanwen get alsps data value is %d\n",psensor_ct);
		}
		psensor_ct = psensor_ct/10;
	} else {
		enable = 0;
		psensor_25_offset = 0,
		psensor_ct = 0;
		pr_err("ps_store_enable(), Please input the number 25 or 1 to get the raw data");
	}

	return count;
}

static ssize_t psenable_show(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", enable);
}
static ssize_t ps25cm_show(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", psensor_25_offset);
}
static ssize_t psct_show(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", psensor_ct);
}
static ssize_t psdata_show(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
	struct alsps_context *cxt = NULL;
	int value = -1;
...
DEVICE_ATTR_RW(psenable);
DEVICE_ATTR_RO(ps25cm);
DEVICE_ATTR_RO(psct);
DEVICE_ATTR_RO(psdata);
