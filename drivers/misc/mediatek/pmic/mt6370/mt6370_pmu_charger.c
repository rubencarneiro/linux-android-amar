...
bool factory_discharge_flag = false;
bool ship_mode_flag = false;
...
static int __mt6370_set_mivr(struct mt6370_pmu_charger_data *chg_data, u32 uV)
{
	...
	if(factory_discharge_flag)
	{
		printk("===============factory discharing=========set mivr return=====\r\n");
		return 0;
	}
	/* Find corresponding reg value */
...
out:
	...
	if (chg_data->hz_mode){
		if(en)
			mt6370_enable_hz(chg_data, false);
		else
...
static int mt6370_enable_power_path(struct charger_device *chg_dev, bool en)
{
	...
	if(factory_discharge_flag)
	{
		printk("===============factory discharing=========power path return=====\r\n");
		return 0;
	}
...
out:
	...
	else
	{
		...
		if(vbus_gpio>0)
		{
			gpio_set_value(vbus_gpio,0);
		}
		gpio_set_value(pogo2_en,0);
	}
	
	return ret;
}

int mt6370_is_otg(void)
{
	return gpio_get_value(vbus_gpio);
}

static int mt6370_factory_discharge(struct charger_device *chg_dev, bool en)
{
	printk("================%s================en[%d]===================\n", __func__, en);
	if(!en)
	{
		factory_discharge_flag = en;
	}
	mt6370_enable_power_path(chg_dev,!en);
	if(en)
	{
		factory_discharge_flag = en;
	}
...
int mt6370_is_pogo_enable(void)
{
	return gpio_get_value(pogo2_en);
}

static int mt6370_pogo_enable(struct charger_device *chg_dev, bool en)
{
	if(en)
		{
			gpio_set_value(pogo2_en,0);
	...
	struct mt6370_pmu_charger_data *chg_data)
{
	...
	ret = of_get_named_gpio(np, "usb,vbus_gpio", 0);
#else
	ret = of_property_read_u32(np, "usb,vbus_gpio_num", &chip->irq_gpio);
#endif
	vbus_gpio = ret;//of_get_named_gpio_flags(node,"usb,vbus_gpio", 0,NULL);
	if (vbus_gpio < 0)
	{
		printk( "vbus-gpio is not available\n");
	}
	else
	{
		gpio_request(vbus_gpio,"vbus-gpio");
		ret = gpio_direction_output(vbus_gpio,0);
	}
	
	pogo2_en = of_get_named_gpio(np, "dock,pogo2_en", 0);
	if(pogo2_en < 0)
	{
		printk("===============Failed to read pogo2_en gpio\n");
	}
	else
	{
		//gpio_request(pogo2_en,"pogo2_en");
		ret = gpio_direction_output(pogo2_en,0);
	}	
...
int mt6370_set_hz_mode(struct charger_device *chg_dev, bool en)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
...
static struct charger_ops mt6370_chg_ops = {
    ...
    .is_otg = mt6370_is_otg,
    .factory_discharge = mt6370_factory_discharge,
    .pogo_enable = mt6370_pogo_enable,
    .is_pogo_enable = mt6370_is_pogo_enable,
	/* PE+/PE+20 */
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (chg_data->psy_desc.type == POWER_SUPPLY_TYPE_USB&& chg_data->psy_usb_type != POWER_SUPPLY_USB_TYPE_DCP)
			val->intval = 500000;
		else
			val->intval = 0;
		break;
...
static void mt6370_pmu_charger_shutdown(struct platform_device *pdev)
{
	...
	if(ship_mode_flag)
	{
		printk("================entry ship mode when shut down start=================\r\n");
		ret = mt6370_pmu_reg_write(chg_data->chip,
					   MT6370_PMU_REG_RSTPASCODE1, 0xA9);
		if (ret < 0) {
			printk("set passcode1 fail\n");
		}
		ret = mt6370_pmu_reg_write(chg_data->chip,
					   MT6370_PMU_REG_RSTPASCODE2, 0x96);
		if (ret < 0) {
			printk("set passcode2 fail\n");
		}
		/* reset all chg/fled/ldo/rgb/bl/db reg and logic */
		ret = mt6370_pmu_reg_write(chg_data->chip,
					     MT6370_PMU_REG_CORECTRL2, 0x7F);
		if (ret < 0) {
			printk("set reset bits fail\n");
		}
		/* disable chg auto sensing */
		mt6370_enable_hidden_mode(chg_data, true);
		ret = mt6370_pmu_reg_clr_bit(chg_data->chip,
			MT6370_PMU_REG_CHGHIDDENCTRL15, 0x01);
		if (ret < 0) {
			printk("set auto sensing disable\n");
		}
		mt6370_enable_hidden_mode(chg_data, false);
		mdelay(50);
		/* enter shipping mode */
		...
		if (ret < 0) {
			printk("enter shipping mode\n");
		}
		printk("================entry ship mode when shut down end=================\r\n");
	}
