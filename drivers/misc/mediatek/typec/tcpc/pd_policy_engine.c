...
int pd_policy_engine_run(struct tcpc_device *tcpc_dev)
{
	...
	if (ret == PE_NEW_EVT_NULL){
		if(tcpc_dev->typec_attach_new == TYPEC_ATTACHED_CUSTOM_SRC &&
			!tcpc_dev->partner) {
		PE_INFO(" typec new state(%d), try register partner\n",tcpc_dev->typec_attach_new);
		tcpc_dev->partner = typec_register_partner(
			pd_port->tcpc_dev->typec_port,
