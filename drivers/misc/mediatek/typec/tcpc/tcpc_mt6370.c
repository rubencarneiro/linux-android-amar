				...
				char *buf)
{
	...
	data = mt6370_i2c_read8(chip->tcpc, TCPC_V10_REG_TCPC_CTRL);
	if (data < 0)
		return data;
...
DEVICE_ATTR(cc_status, S_IRUGO, cc_status_show, NULL);
...
static int mt6370_tcpcdev_init(struct mt6370_chip *chip, struct device *dev)
{
	...
	if (ret < 0) {
		dev_err(dev, "====================failed to create dev_attr_cc_status\n");
	}
	return 0;
}
