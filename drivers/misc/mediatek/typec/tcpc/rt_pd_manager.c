				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PLUG_OUT:
		...
		if (rpmd->tcpc_kpoc) {
			...
			{
			    kernel_power_off();
				printk("======leon======power off charger========power off===========");
			}
			else
			{
				printk("=======leon=====power off charger========dock in missed power off===========");
			}
		}
		break;
		case TCP_NOTIFY_POGO_EN:
			pr_err("%s pogo en = %d\n", __func__, noti->en_state.en);
			/* active low */
			gpio_set_value(rpmd->pogo_en_gpio, !noti->en_state.en);
			break;
		case TCP_NOTIFY_IRQ_ENABLED:
			pr_err("%s irq enabled\n", __func__);
			if (rpmd->tcpc_kpoc) {
				tcpm_typec_change_role(rpmd->tcpc_dev, TYPEC_ROLE_SNK);
				tcpm_set_dpm_caps(rpmd->tcpc_dev,
						  tcpm_inquire_dpm_caps(rpmd->tcpc_dev) |
...
static int rt_pd_manager_probe(struct platform_device *pdev)
{
    ...
    if (node == NULL) {
		pr_err("%s device of node not exist\n", __func__);
		return -ENODEV;
	}
	...
	if (IS_ERR(rpmd->chg_psy)) {
		...
		ret = of_get_named_gpio(node, "pogo_en_gpio", 0);
		if (ret < 0) {
			pr_err("%s get pogo_en_gpio fail(%d)\n", __func__, ret);
			return ret;
		}
		rpmd->pogo_en_gpio = ret;
	#else
		ret = of_property_read_u32(node, "pogo_en_gpio_num", &rpmd->pogo_en_gpio);
		if (ret < 0) {
			pr_err("%s get pogo_en_gpio_num fail(%d)\n", __func__, ret);
			return ret;
		}
	#endif
		pr_info("%s pogo en gpio = %d\n", __func__, rpmd->pogo_en_gpio);
		ret = gpio_request_one(rpmd->pogo_en_gpio, GPIOF_OUT_INIT_LOW, "pogo_en_gpio");
		if (ret < 0) {
	...
	{
		tcpm_typec_change_role(rpmd->tcpc_dev, TYPEC_ROLE_SNK);
		printk("===========leon===========power off charger===make  TYPEC_ROLE_SNK only=====\r\n");
		tcpm_set_dpm_caps(rpmd->tcpc_dev, tcpm_inquire_dpm_caps(rpmd->tcpc_dev) |
				  DPM_CAP_PR_SWAP_REJECT_AS_SRC);
	}
