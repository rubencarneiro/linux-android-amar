...
static inline bool typec_cc_change_source_entry(struct tcpc_device *tcpc_dev)
{
	...
	tcpci_notify_pogo_en(tcpc_dev, src_remove);
...
static inline bool typec_cc_change_sink_entry(struct tcpc_device *tcpc_dev)
{
	...
	tcpci_notify_pogo_en(tcpc_dev, snk_remove);
...
#endif /* CONFIG_WATER_DETECTION */
    u8 tcpc_state_backup = tcpc_dev->typec_state;
...
//	TYPEC_INFO("[CC_Alert] %d/%d\r\n", typec_get_cc1(), typec_get_cc2());
	TYPEC_INFO("[CC_Alert] %d/%d, state(%d)", typec_get_cc1(), typec_get_cc2(), tcpc_state_backup);
...
#ifdef CONFIG_TYPEC_CAP_NORP_SRC
	if (tcpc_state_backup == typec_attached_norp_src &&
			 (TYPEC_CC_VOLT_RD == typec_get_cc1() ||
			 TYPEC_CC_VOLT_RD == typec_get_cc2())) {
			 TYPEC_INFO("TCPC-DBG: OTG in but NORP_SRC, disable pogo\n");
			 tcpci_notify_pogo_en(tcpc_dev, 0);
		 }
...
static inline void typec_handle_sink_wait_bc12(struct tcpc_device *tcpc_dev)
{
	...
	TYPEC_INFO("type=%d, ret,chg_type=%d,%d, count=%d\r\n", type,
		ret, prop.intval, tcpc_dev->sink_wait_bc12_count);
