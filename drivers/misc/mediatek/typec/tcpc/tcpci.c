...
#endif	/* CONFIG_USB_POWER_DELIVERY */
int tcpci_notify_pogo_en(struct tcpc_device *tcpc, bool en)
{
	struct tcp_notify tcp_noti;
	int ret;
	...
	tcp_noti.en_state.en = en;
	ret = tcpc_check_notify_time(tcpc, &tcp_noti,
		TCP_NOTIFY_IDX_VBUS, TCP_NOTIFY_POGO_EN);
	return ret;
}

int tcpci_notify_irq_enabled(struct tcpc_device *tcpc)
{
	struct tcp_notify tcp_noti;
	int ret;

	ret = tcpc_check_notify_time(tcpc, &tcp_noti,
		TCP_NOTIFY_IDX_MISC, TCP_NOTIFY_IRQ_ENABLED);
	return ret;
}
