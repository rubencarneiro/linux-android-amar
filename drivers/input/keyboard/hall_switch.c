...
irqreturn_t hall_irq_handler(int irq, void *dev_id)
{
		...
		//input_report_key(input_dev, HALL_KEY, !state);
...
static int hall_switch_probe (struct platform_device *pdev)
{
		...
		__set_bit(EV_SW, hall_dev_data->input_dev->evbit);
		__set_bit(SW_LID, hall_dev_data->input_dev->swbit);
		//__set_bit(EV_KEY,hall_dev_data->input_dev->evbit);
		//__set_bit(HALL_KEY, hall_dev_data->input_dev->keybit);
		...
		enable_irq_wake(hall_dev_data->irq);
