...
#define CONFIG_META_PWRKEY_LONG_POWEROFF
...
#ifdef CONFIG_META_PWRKEY_LONG_POWEROFF
static struct task_struct *meta_power_off;
...
int mtk_pmic_get_boot_mode(void)
{
...
#ifdef CONFIG_META_PWRKEY_LONG_POWEROFF
	if (mtk_pmic_get_boot_mode() == META_BOOT && meta_poweroff_timer_started == true) {
		if(hrtimer_cancel(&meta_poweroff_timer)) {
			meta_poweroff_timer_started = false;
			dev_info(info->keys->dev, "meta_poweroff time is cancel");
		}
	}
#endif
...
#ifdef CONFIG_META_PWRKEY_LONG_POWEROFF
	if (mtk_pmic_get_boot_mode() == META_BOOT) {
		hr_time = ktime_set(META_PWRKEY_LONG_PRESS_POWEROFF_TIME,0);
		hrtimer_start(&meta_poweroff_timer, hr_time, HRTIMER_MODE_REL);
		dev_info(info->keys->dev,"hrtimer is open \n");
...
#ifdef CONFIG_META_PWRKEY_LONG_POWEROFF
static int power_off(void *data)
{
	kernel_power_off();
...
{
	wake_up_process(meta_power_off);
...
static int mtk_pmic_keys_probe(struct platform_device *pdev)
{
	...
	if (mtk_pmic_get_boot_mode() == META_BOOT) {
		...
		//kernel_power_off();
		meta_power_off = kthread_create(power_off, NULL, "meta_power_off");
		if(IS_ERR(meta_power_off)) {
			err = PTR_ERR(meta_power_off);
			printk("%s:kthread_create failed,ret=%d\n", __func__, err);
			meta_power_off = NULL;
			return err;
		}
	}
