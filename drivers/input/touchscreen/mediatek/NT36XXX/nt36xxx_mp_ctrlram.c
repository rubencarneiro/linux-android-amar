 ...
 *
 * $Revision: 79836 $
 * $Date: 2021-03-31 14:56:32 +0800 (周三, 31 3月 2021) $
 *
...
static int32_t TestResult_I2C_Comm = 0;
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	nvt_mp_printf("I2C Communication Test");
	if (TestResult_I2C_Comm == 0) {
		test_result_bmp[0] = 'P';
		nvt_mp_printf(" PASS!\n\n");
	} else {
		test_result_bmp[0] = 'F';
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_get_fw_info()) {
		...
		TestResult_I2C_Comm = -1;
		TestResult_FW_Rawdata = -1;
		TestResult_Open = -1;
		TestResult_Short = -1;
		TestResult_Noise = -1;
		goto err_nvt_i2c_comm;
		//return -EAGAIN;
	} else {
		TestResult_I2C_Comm = 0;
	}
...
{
	NVT_proc_selftest_entry = proc_create("self_test", 0444, proc_android_touch_entry, &nvt_selftest_fops);
...
void nvt_mp_proc_deinit(void)
{
	...
	if (NVT_proc_selftest_entry != NULL) {
		remove_proc_entry("android_touch/self_test", NULL);
