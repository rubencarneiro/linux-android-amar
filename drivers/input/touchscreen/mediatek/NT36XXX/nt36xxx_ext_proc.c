 ...
 *
 * $Revision: 77558 $
 * $Date: 2021-02-04 14:27:53 +0800 (周四, 04 2月 2021) $
 *
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_gesture_entry = proc_create(GESTURE_CONTROL, 0644, proc_android_touch_entry, &nvt_gesture_fops);
	if (NVT_proc_gesture_entry == NULL) {
		NVT_ERR("create proc/android_touch/gesture_control Failed!\n");
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/android_touch/gesture_control Succeeded!\n");
	}
...
void nvt_extra_proc_deinit(void)
{
	...
	if (NVT_proc_gesture_entry != NULL) {
		remove_proc_entry("android_touch/gesture_control", NULL);
		NVT_proc_gesture_entry = NULL;
		NVT_LOG("Removed /proc/android_touch/gesture_control \n");
	}
}
#endif
