...
static void hx83102ab_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
	...
	do {
		if (SMWP_enable) {
			kp_himax_in_parse_assign_cmd(fw_func_handshaking_pwd,
				tmp_data, 4);
...
static void hx83102e_sense_on(uint8_t FlashMode)
{
	...
	if (!FlashMode) {
		...
		ret = kp_himax_bus_write(pic_op->adr_i2c_psw_lb[0],
			tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
		if (ret < 0)
			E("%s: i2c access fail!\n", __func__);

		ret = kp_himax_bus_write(pic_op->adr_i2c_psw_ub[0],
			tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
		if (ret < 0)
			E("%s: i2c access fail!\n", __func__);
		...
		int type)
{
	...
	case 5:
		...
		break;
	case 6:
		kp_himax_in_parse_assign_cmd(hx83102d_zf_data_bor_prevent_info,
			tmp_data, 4);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_data, 0xC9E0, 32);
		break;
	case 7:
		kp_himax_in_parse_assign_cmd(hx83102d_zf_data_notch_info,
			tmp_data, 4);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_data,
			0xCA00, 128);
		break;
	case 8:
		kp_himax_in_parse_assign_cmd(hx83102d_zf_func_info_en,
			tmp_data, 4);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_data, 0xCB00, 12);
		break;
	case 9:
		kp_himax_in_parse_assign_cmd(hx83102d_zf_po_sub_func,
			tmp_data, 4);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_data,
			0xA000, HX4K);
		break;
...
static int hx83102d_0f_overlay(int ovl_type, int mode)
{
	...
	for (i = 1; i < part_num; i++) {
		...
		&& buf[1] == sram_addr[1]
		&& buf[0] == sram_addr[0]) {
			zf_info_arr[j].write_size = buf[7] << 24
						| buf[6] << 16
						| buf[5] << 8
						| buf[4];
			zf_info_arr[j].fw_addr = buf[11] << 24
						| buf[10] << 16
						| buf[9] << 8
						| buf[8];
			/*I("j = %d; write_size = %d; fw_addr = %d\n", j,
				zf_info_arr[j].write_size,
				zf_info_arr[j].fw_addr);*/
			j++;
		}
	}
...
static void hx83102a_read_ic_ver(void)
{
	...
	tmp_data[2] = 0x10;
	tmp_data[3] = 0x2A;
	kp_himax_in_parse_assign_cmd(hx83102_ic_b9_en, tmp_addr,
			sizeof(tmp_addr));
	kp_g_core_fp->fp_register_write(tmp_addr, DATA_LEN_4,
			tmp_data,  false);

	/* Read DD data */
	kp_himax_in_parse_assign_cmd(hx83102_ic_fw, tmp_addr,
			sizeof(tmp_addr));
...
static void hx83102b_read_ic_ver(void)
{
	...
	(*kp_ic_data)->vendor_ic_ver = 0xFFFF;
	/* Enable read DD */
	kp_himax_in_parse_assign_cmd(hx83102b_ic_osc_en, tmp_addr,
			sizeof(tmp_addr));
	kp_g_core_fp->fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
	tmp_data[0] = 0x01;
...
static void hx83102d_read_ic_ver(void)
{
	...
	(*kp_ic_data)->vendor_ic_ver = 0xFFFF;
	/* Enable read DD */
	kp_himax_in_parse_assign_cmd(hx83102d_ic_osc_pw, tmp_addr,
			sizeof(tmp_addr));
	kp_g_core_fp->fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
	tmp_data[0] = 0xA5;
	kp_g_core_fp->fp_register_write(tmp_addr, DATA_LEN_4, tmp_data, false);

	kp_himax_in_parse_assign_cmd(hx83102d_ic_osc_en, tmp_addr,
			sizeof(tmp_addr));
	kp_g_core_fp->fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
	tmp_data[0] = 0xDD;
	kp_g_core_fp->fp_register_write(tmp_addr, DATA_LEN_4, tmp_data, false);
...
static void hx83102_dynamic_fw_name(uint8_t ic_name)
{
	...
	/* Cut 1 */
	if (((*kp_ic_data)->vendor_ic_ver == 0x00)
	|| ((*kp_ic_data)->vendor_ic_ver == 0x21)) {
		memcpy(firmware_name, "Himax_firmware_1.bin",
				sizeof(char)*strlen("Himax_firmware_1.bin"));
	/* Cut 2 */
	} else if (((*kp_ic_data)->vendor_ic_ver == 0x01)
	|| ((*kp_ic_data)->vendor_ic_ver == 0x11)) {
		memcpy(firmware_name, "Himax_firmware_2.bin",
				sizeof(char)*strlen("Himax_firmware_2.bin"));
	} else { /* Original */
...
{
	I("%s:Entering!\n", __func__);
	kp_himax_in_parse_assign_cmd(hx83102ab_fw_addr_sorting_mode_en,
			(*kp_pfw_op)->addr_sorting_mode_en,
			sizeof((*kp_pfw_op)->addr_sorting_mode_en));
	kp_himax_in_parse_assign_cmd(hx83102ab_fw_addr_selftest_addr_en,
			(*kp_pfw_op)->addr_selftest_addr_en,
			sizeof((*kp_pfw_op)->addr_selftest_addr_en));
#ifdef HX_ZERO_FLASH
	kp_himax_in_parse_assign_cmd(hx83102ab_data_adc_cfg_1,
			(*kp_pzf_op)->data_adc_cfg_1,
			sizeof((*kp_pzf_op)->data_adc_cfg_1));
#endif

	if (ic_name == 0x2a) {
		kp_himax_in_parse_assign_cmd(hx83102a_data_df_rx,
				(*kp_pdriver_op)->data_df_rx,
				sizeof((*kp_pdriver_op)->data_df_rx));
		kp_himax_in_parse_assign_cmd(hx83102a_data_df_tx,
				(*kp_pdriver_op)->data_df_tx,
				sizeof((*kp_pdriver_op)->data_df_tx));
		kp_himax_in_parse_assign_cmd(hx83102a_data_df_x_res,
				(*kp_pdriver_op)->data_df_x_res,
				sizeof((*kp_pdriver_op)->data_df_x_res));
		kp_himax_in_parse_assign_cmd(hx83102a_data_df_y_res,
				(*kp_pdriver_op)->data_df_y_res,
				sizeof((*kp_pdriver_op)->data_df_y_res));
	} else {
		kp_himax_in_parse_assign_cmd(hx83102b_data_df_x_res,
				(*kp_pdriver_op)->data_df_x_res,
				sizeof((*kp_pdriver_op)->data_df_x_res));
		kp_himax_in_parse_assign_cmd(hx83102b_data_df_y_res,
				(*kp_pdriver_op)->data_df_y_res,
				sizeof((*kp_pdriver_op)->data_df_y_res));
	}
}
...
{
	I("%s:Entering!\n", __func__);
	kp_himax_in_parse_assign_cmd(hx83102d_fw_addr_raw_out_sel,
			(*kp_pfw_op)->addr_raw_out_sel,
			sizeof((*kp_pfw_op)->addr_raw_out_sel));
#if defined(HX_ZERO_FLASH)
	kp_himax_in_parse_assign_cmd(hx83102d_zf_data_sram_start_addr,
			(*kp_pzf_op)->data_sram_start_addr,
			sizeof((*kp_pzf_op)->data_sram_start_addr));
	kp_himax_in_parse_assign_cmd(hx83102d_zf_data_adc_cfg_1,
			(*kp_pzf_op)->data_adc_cfg_1,
			sizeof((*kp_pzf_op)->data_adc_cfg_1));
	kp_himax_in_parse_assign_cmd(hx83102d_zf_data_adc_cfg_2,
			(*kp_pzf_op)->data_adc_cfg_2,
			sizeof((*kp_pzf_op)->data_adc_cfg_2));
	kp_himax_in_parse_assign_cmd(hx83102d_zf_data_adc_cfg_3,
			(*kp_pzf_op)->data_adc_cfg_3,
			sizeof((*kp_pzf_op)->data_adc_cfg_3));
#endif
	kp_himax_in_parse_assign_cmd(hx83102d_data_df_x_res,
			(*kp_pdriver_op)->data_df_x_res,
			sizeof((*kp_pdriver_op)->data_df_x_res));
	kp_himax_in_parse_assign_cmd(hx83102d_data_df_y_res,
			(*kp_pdriver_op)->data_df_y_res,
			sizeof((*kp_pdriver_op)->data_df_y_res));
	kp_himax_in_parse_assign_cmd(hx83102d_data_df_y_res,
			(*kp_pdriver_op)->data_df_y_res,
			sizeof((*kp_pdriver_op)->data_df_y_res));
	kp_himax_in_parse_assign_cmd(hx83102d_adr_osc_en,
			(*kp_pic_op)->adr_osc_en,
			sizeof((*kp_pic_op)->adr_osc_en));
	kp_himax_in_parse_assign_cmd(hx83102d_adr_osc_pw,
...
static void himax_hx83102e_reg_re_init(void)
{
	...
	kp_himax_in_parse_assign_cmd(hx83102e_fw_addr_raw_out_sel,
			(*kp_pfw_op)->addr_raw_out_sel,
			sizeof((*kp_pfw_op)->addr_raw_out_sel));
	kp_himax_in_parse_assign_cmd(hx83102e_data_df_rx,
			(*kp_pdriver_op)->data_df_rx,
			sizeof((*kp_pdriver_op)->data_df_rx));
	kp_himax_in_parse_assign_cmd(hx83102e_data_df_tx,
			(*kp_pdriver_op)->data_df_tx,
			sizeof((*kp_pdriver_op)->data_df_tx));
	kp_himax_in_parse_assign_cmd(hx83102e_data_df_x_res,
			(*kp_pdriver_op)->data_df_x_res,
			sizeof((*kp_pdriver_op)->data_df_x_res));
	kp_himax_in_parse_assign_cmd(hx83102e_data_df_y_res,
			(*kp_pdriver_op)->data_df_y_res,
			sizeof((*kp_pdriver_op)->data_df_y_res));
	kp_himax_in_parse_assign_cmd(hx83102e_ic_adr_tcon_rst,
...
static int __init himax_hx83102_init(void)
{
	int ret = 0;
	I("lcm_detect_name %s\n", lcm_detect_name);
	if ( !lcm_detect_name || !strcmp(lcm_detect_name, "mipi_nt36523a_inx_video_800p")) {
		I("%s:lcm is not himax\n", __func__);
		return -1;
	}
