...
#else
char *i_CTPM_firmware_name_boe = "Himax_firmware_boe.bin";
char *i_CTPM_firmware_name_inx = "Himax_firmware_inx.bin";
#endif
...
#define CHARGER_NAME "mt6370_pmu_charger"
/*charger_detect_function*/
static int charger_notifier_callback(struct notifier_block *nb, unsigned long val, void *v);
static void himax_charger_notify_work(struct work_struct *work);
#endif
...
#ifdef HX_SMART_WAKEUP
#define HIMAX_PROC_SMWP_FILE "gesture_control"
...
static void himax_charger_notify_work(struct work_struct *work)
{
	...
	if (NULL == work) {
		E("%s:  parameter work are null!\n", __func__);
		return;
	}

	if (USB_DETECT_IN == USB_detect_flag) {
		if (mutex_lock_interruptible(&input_dev->mutex)) {
			E("Failed to lock in mutex_lock_interruptible(&input_dev->lock).\n");
			return;
		}
		...
		if (mutex_lock_interruptible(&input_dev->mutex)) {
			E("Failed to lock in mutex_lock_interruptible(&ts->lock).\n");
			return;
		}
...
static int charger_notifier_callback(struct notifier_block *nb,unsigned long val, void *v)
{
	...
	struct himax_usb_charger_detection *charger_detection =
		container_of(nb, struct himax_usb_charger_detection, charger_notif);
	union power_supply_propval prop;

	psy= power_supply_get_by_name(CHARGER_NAME);
	if (!psy){
		return -EINVAL;
		E("Couldn't get usbpsy\n");
	}

	if (!strcmp(psy->desc->name, CHARGER_NAME)){
			...
			if (ret < 0) {
				...
				if(USB_detect_flag != charger_detection->usb_connected) {
					...
					if (!ts_data->suspended){
						queue_work(charger_detection->himax_charger_notify_wq,&charger_detection->charger_notify_work);
					}
				}
			}
		}
...
int Himax_gesture_status(void)
{
	if (!private_ts)
		return 0;

	return private_ts->SMWP_enable;
}
...
static void calcDataSize(void)
{
	...
	I("%s: coord_data_size: %d", __func__, ts_data->coord_data_size);
	I("%s: area_data_size:%d", __func__, ts_data->area_data_size);
	I("%s: raw_data_frame_size:%d", __func__, ts_data->raw_data_frame_size);
	I("%s: raw_data_nframes:%d", __func__, ts_data->raw_data_nframes);
}
...
static int i_get_FW(void)
{
	...
	if(ts->FW_TYPE == BOE_FW){
		...
		ret = request_firmware(&image, i_CTPM_firmware_name_boe, private_ts->dev);
	}else if(ts->FW_TYPE == INX_FW){
		I("file name = %s\n", i_CTPM_firmware_name_inx);
		ret = request_firmware(&image, i_CTPM_firmware_name_inx, private_ts->dev);
	}
	else{
		I("file name = %s\n", i_CTPM_firmware_name_boe);
		ret = request_firmware(&image, i_CTPM_firmware_name_boe, private_ts->dev);
...
static int himax_wake_event_parse(struct himax_ts_data *ts, int ts_status)
{
	...
	&& buf[GEST_PTLG_ID_LEN + 1] == GEST_PTLG_HDR_ID2) {
		...
		if (gest_pt_cnt) {
			...
			I("gest_point x_min = %d\n", tmp_min_x);
			I("gest_point x_max = %d\n", tmp_max_x);
			I("gest_point y_min = %d\n", tmp_min_y);
			I("gest_point y_max = %d\n", tmp_max_y);
...
int himax_report_data_init(void)
{
	...
	* ic_data->HX_RX_NUM
	...
	I("%s:rawdata_frame_size = %d\n", __func__,
			hx_touch_data->rawdata_frame_size);
	I("%s:ic_data->HX_MAX_PT:%d\n", __func__, ic_data->HX_MAX_PT);
	I("%s:hx_raw_cnt_max:%d\n", __func__, hx_touch_data->raw_cnt_max);
	I("%s:hx_raw_cnt_rmd:%d\n", __func__, hx_touch_data->raw_cnt_rmd);
	I("%s:g_hx_rawdata_size:%d,\n", __func__, hx_touch_data->rawdata_size);
	I("%s:touch_info_size:%d\n", __func__, hx_touch_data->touch_info_size);
...
/* SMWP */
	...
	if (check_sum_cal % 0x100 != 0) {
		I("[HIMAX TP MSG] point data_checksum not match ");
		I("check_sum_cal: 0x%02X\n", check_sum_cal);
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if (valid) {
			if (g_ts_dbg != 0) {
				I("g_target_report_data->x[i]=%d,",
					g_target_report_data->x[i]);
				I("g_target_report_data->y[i]=%d,",
					g_target_report_data->y[i]);
				I("g_target_report_data->w[i]=%d\n",
	...
	if (ts->cable_config) {
		if (!atomic_read(&ts->suspend_mode)) {
			if (connect_status != ts->usb_connected) {
				...
				I("%s:Cable sts is same as prev one,ignore\n",
						__func__);
		} else {
			if (connect_status)
				ts->usb_connected = 0x01;
...
static void himax_fb_register(struct work_struct *work)
{
	...
	struct himax_ts_data *ts = container_of(work,
			struct himax_ts_data, work_att.work);
...
FW_force_upgrade:
	...
	if (ts->charger_detection_enable){
		ts->charger_detection = kzalloc(sizeof(struct himax_usb_charger_detection), GFP_KERNEL);
		if (ts->charger_detection == NULL) {
			E("failed to allocated memory for usb_charger_detection\n");
	...
	if (ts->charger_detection) {
		...
		I("charger_detection on\n");
		ts->charger_detection->usb_connected = 0;
		ts->charger_detection->himax_charger_notify_wq = create_singlethread_workqueue("himax_charger_wq");

		if (!ts->charger_detection->himax_charger_notify_wq){
			E("allocate himax_charger_notify_wq failed\n");
			goto err_charger_notify_wq_failed;
		}
		INIT_WORK(&ts->charger_detection->charger_notify_work, himax_charger_notify_work);
	...
	I("%s\n", mtkfb_lcm_name);
	if (strcmp(mtkfb_lcm_name,"mipi_hx83102e_boe_video_800p") == 0) {
		ts->FW_TYPE = BOE_FW;
		hq_regiser_hw_info(HWID_LCM, "Vendor:BOE\nIC:himax\nLCD_FW:04");
		hq_regiser_hw_info(HWID_CTP, "Vendor:BOE\nIC:himax\nTP_FW:08");
	} else if (strcmp(mtkfb_lcm_name,"mipi_hx83102e_inx_video_800p") == 0) {
		ts->FW_TYPE = INX_FW;
		hq_regiser_hw_info(HWID_LCM, "Vendor:INX\nIC:himax\nLCD_FW:05");
		hq_regiser_hw_info(HWID_CTP, "Vendor:INX\nIC:himax\nTP_FW:5A");
	} else {
		ts->FW_TYPE = BOE_FW;
	}
