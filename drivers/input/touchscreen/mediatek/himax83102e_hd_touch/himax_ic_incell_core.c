...
static uint32_t himax_mcu_check_CRC(uint8_t *start_addr, int reload_length)
{
	...
	do {
		...
		if ((tmp_data[0] & 0x01) != 0x01) {
			...
			I("%s: tmp_data[3]=%X, tmp_data[2]=%X\n",
				__func__, tmp_data[3], tmp_data[2]);
			I("%s: tmp_data[1]=%X, tmp_data[0]=%X\n",
		...
		uint8_t storage_type)
{
	...
	do {
		...
		I("%s: back_data[3]=0x%02X\n", __func__, back_data[3]);
		I("%s: back_data[2]=0x%02X\n", __func__, back_data[2]);
		I("%s: back_data[1]=0x%02X\n", __func__, back_data[1]);
		I("%s: back_data[0]=0x%02X\n", __func__, back_data[0]);
		cnt--;
...
static int himax_mcu_chip_self_test(void)
{
	...
	for (i = 0; i < 1000; i++) {
		g_core_fp.fp_register_read(pfw_op->addr_selftest_addr_en, 4,
				tmp_data, 0);
		I("%s: tmp_data[0] = 0x%02X, cnt=%d\n",
				__func__, tmp_data[0], i);
		I("%s: tmp_data[1] = 0x%02X, cnt=%d\n",
				__func__, tmp_data[1], i);
		I("%s: tmp_data[2] = 0x%02X, cnt=%d\n",
				__func__, tmp_data[2], i);
		I("%s: tmp_data[3] = 0x%02X, cnt=%d\n",
				__func__, tmp_data[3], i);
		usleep_range(10000, 11000);
...
static void himax_mcu_idle_mode(int disable)
{
	...
	do {
		...
		I("%s:After turn ON/OFF IDLE Mode[0]=0x%02X\n", __func__,
			tmp_data[0]);
		I("%s:After turn ON/OFF IDLE Mode[1]=0x%02X\n", __func__,
			tmp_data[1]);
		I("%s:After turn ON/OFF IDLE Mode[2]=0x%02X\n", __func__,
			tmp_data[2]);
		I("%s:After turn ON/OFF IDLE Mode[3]=0x%02X\n", __func__,
			tmp_data[3]);
...
static int himax_mcu_read_ic_trigger_type(void)
{
	...
	g_core_fp.fp_register_read(pfw_op->addr_trigger_addr,
			DATA_LEN_4, tmp_data, 0);
...
static void himax_mcu_read_FW_ver(void)
{
	...
	I("%s : data[0]=0x%2.2X\n", __func__, data[0]);
	I("%s : data[1]=0x%2.2X\n", __func__, data[1]);
	I("%s : data_2[0]=0x%2.2X\n", __func__, data_2[0]);
	I("%s : data_2[1]=0x%2.2X\n", __func__, data_2[1]);
	I("reload_status=%d\n", reload_status);
	/**
	 * Read FW version
	 */
...
{
	I("%s:Now tmp_data[3]=0x%02X\n", __func__, tmp_data[3]);
	I("%s:Now tmp_data[2]=0x%02X\n", __func__, tmp_data[2]);
	I("%s:Now tmp_data[1]=0x%02X\n", __func__, tmp_data[1]);
	I("%s:Now tmp_data[0]=0x%02X\n", __func__, tmp_data[0]);
...
static int himax_mcu_switch_mode(int mode)
{
	...
	/* clean up FW status */
	g_core_fp.fp_register_write(psram_op->addr_rawdata_addr,
			DATA_LEN_4, psram_op->addr_rawdata_end, 0);
	tmp_data[3] = 0x00;
	tmp_data[2] = 0x00;
...
static uint8_t himax_mcu_read_DD_status(uint8_t *cmd_set, uint8_t *tmp_data)
{
	...
	I("%s: cmd_set[0] = 0x%02X\n", __func__, cmd_set[0]);
	I("%s: cmd_set[1] = 0x%02X\n", __func__, cmd_set[1]);
	I("%s: cmd_set[2] = 0x%02X\n", __func__, cmd_set[2]);
	I("%s: cmd_set[3] = 0x%02X\n", __func__, cmd_set[3]);
		...
		int len, bool change_iref)
{
	...
	/*modify FW Number show error after upgrade */
	/* reset reload done*/
	g_core_fp.fp_register_write(pdriver_op->addr_fw_define_2nd_flash_reload, DATA_LEN_4,pfw_op->data_clear, 0);
...
static bool himax_mcu_flash_lastdata_check(uint32_t size)
{
	...
	&& (!flash_tmp_buffer[flash_page_len-1])) {
		...
		I("flash_buffer[FFFC]=0x%2X\n",
			flash_tmp_buffer[flash_page_len-4]);
		I("flash_buffer[FFFD]=0x%2X\n",
			flash_tmp_buffer[flash_page_len-3]);
		I("flash_buffer[FFFE]=0x%2X\n",
			flash_tmp_buffer[flash_page_len-2]);
		I("flash_buffer[FFFF]=0x%2X\n",
			flash_tmp_buffer[flash_page_len-1]);
		goto FAIL;/*FAIL*/
...
static bool himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	I("%s: tmp_data[1] = 0x%02X\n", __func__,
		psram_op->addr_rawdata_addr[1]);
	I("%s: tmp_data[2] = 0x%02X\n", __func__,
		psram_op->addr_rawdata_addr[2]);
	I("%s: tmp_data[3] = 0x%02X\n", __func__,
		...
		uint8_t *flash_tmp_buffer)
{
	...
	I("Now start addr: tmp_addr[0]=0x%2X\n", tmp_addr[0]);
	I("Now start addr: tmp_addr[1]=0x%2X\n", tmp_addr[1]);
	I("Now start addr: tmp_addr[2]=0x%2X\n", tmp_addr[2]);
	I("Now start addr: tmp_addr[3]=0x%2X\n", tmp_addr[3]);
...
static int hx_read_guest_info(void)
{
	...
	custom_info_temp++) {
		checksum = himax_guest_info_read(panel_color_addr
			+ custom_info_temp
			* flash_page_len,
...
{
	/*Diable Flash Reload*/
	g_core_fp.fp_register_write(pzf_op->addr_dis_flash_reload,
...
void himax_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
	...
	tmp_addr[1] = addr[1];
	tmp_addr[0] = addr[0];
	I("%s, write addr tmp_addr[3]=0x%2.2X\n", __func__, tmp_addr[3]);
	I("%s, write addr tmp_addr[2]=0x%2.2X\n", __func__, tmp_addr[2]);
	I("%s, write addr tmp_addr[1]=0x%2.2X\n", __func__, tmp_addr[1]);
	I("%s, write addr tmp_addr[0]=0x%2.2X\n", __func__, tmp_addr[0]);
...
int hx_parse_bin_cfg_data(const struct firmware *fw_entry,
		...
		uint8_t *sram_min, uint8_t *ovl_sidx)
{
	int part_num = 0;
	int i = 0;
	uint8_t buf[16];
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	if (ret < 0) {
		...
		I("%s: Couldn't find userspace FW,\n", __func__);
		I("use embedded FW(size:%zu) instead.\n", g_embedded_fw.size);
#else
...
int himax_mcu_0f_operation_dirly(void)
{
	...
	if(ts->FW_TYPE == BOE_FW){
		I("%s, Entering\n", __func__);
		I("file name = %s\n", i_CTPM_firmware_name_boe);
		ret = request_firmware(&fw_entry, i_CTPM_firmware_name_boe,
		private_ts->dev);
	}else if(ts->FW_TYPE == INX_FW){
		I("%s, Entering\n", __func__);
		I("file name = %s\n", i_CTPM_firmware_name_inx);
		ret = request_firmware(&fw_entry, i_CTPM_firmware_name_inx,
		private_ts->dev);
	}else{
		I("%s, Entering\n", __func__);
		I("file name = %s\n", i_CTPM_firmware_name_boe);
...
#if defined(__EMBEDDED_FW__)
		fw_entry = &g_embedded_fw;
		I("%s: Couldn't find userspace FW, ", __func__);
		I("use embedded FW(size:%zu) instead.\n", g_embedded_fw.size);
#else
		E("%s, fail in line%d error code=%d,file maybe fail\n",
				__func__, __LINE__, ret);
		...
		uint8_t *addr, int start_index, int read_len)
{
	...
	tmp_addr[1] = addr[1];
	tmp_addr[0] = addr[0];
	I("%s, read addr tmp_addr[3]=0x%2.2X\n", __func__, tmp_addr[3]);
	I("%s, read addr tmp_addr[2]=0x%2.2X\n", __func__, tmp_addr[2]);
	I("%s, read addr tmp_addr[1]=0x%2.2X\n", __func__, tmp_addr[1]);
	I("%s, read addr tmp_addr[0]=0x%2.2X\n", __func__, tmp_addr[0]);
...
void himax_mcu_read_all_sram(uint8_t *addr, int read_len)
{
	...
	I("%s, NOW addr tmp_addr[3]=0x%2.2X\n", __func__, tmp_addr[3]);
	I("%s, NOW addr tmp_addr[2]=0x%2.2X\n", __func__, tmp_addr[2]);
	I("%s, NOW addr tmp_addr[1]=0x%2.2X\n", __func__, tmp_addr[1]);
	I("%s, NOW addr tmp_addr[0]=0x%2.2X\n", __func__, tmp_addr[0]);
...
#if defined(__EMBEDDED_FW__)
		fw_entry = &g_embedded_fw;
		I("%s:no find userspace FW, use emb FW(size:%zu) instead.\n",
...
int himax_mcu_in_cmd_struct_init(void)
{
	...
	I("%s: Entering!\n", __func__);
	himax_mcu_cmd_struct_free =(void(*)(void))himax_mcu_in_cmd_struct_free;
...
void himax_mcu_in_cmd_init(void)
{
	...
	himax_in_parse_assign_cmd(ic_cmd_rst, pic_op->data_rst,
		sizeof(pic_op->data_rst));
	himax_in_parse_assign_cmd(ic_adr_osc_en, pic_op->adr_osc_en,
		sizeof(pic_op->adr_osc_en));
	himax_in_parse_assign_cmd(ic_adr_osc_pw, pic_op->adr_osc_pw,
	...
	himax_in_parse_assign_cmd(fw_addr_cus_info,
		pfw_op->addr_cus_info,
		sizeof(pfw_op->addr_cus_info));
	himax_in_parse_assign_cmd(fw_addr_proj_info,
	...
	himax_in_parse_assign_cmd(fw_data_safe_mode_release_pw_active,
		pfw_op->data_safe_mode_release_pw_active,
		sizeof(pfw_op->data_safe_mode_release_pw_active));
	himax_in_parse_assign_cmd(fw_data_clear,
		pfw_op->data_clear,
		sizeof(pfw_op->data_clear));
	himax_in_parse_assign_cmd(fw_data_clear,
		pfw_op->data_clear,
		sizeof(pfw_op->data_clear));
	himax_in_parse_assign_cmd(fw_data_fw_stop,
		pfw_op->data_fw_stop,
	...
	himax_in_parse_assign_cmd(driver_data_df_rx,
		pdriver_op->data_df_rx,
		sizeof(pdriver_op->data_df_rx));
	himax_in_parse_assign_cmd(driver_data_df_tx,
		pdriver_op->data_df_tx,
		sizeof(pdriver_op->data_df_tx));
	himax_in_parse_assign_cmd(driver_data_df_pt,
	...
	himax_in_parse_assign_cmd(zf_data_fw_cfg_3,
