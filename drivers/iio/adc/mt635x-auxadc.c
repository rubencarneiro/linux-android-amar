...
struct iio_dev *g_indio_dev = NULL;
...
int mt6357_auxadc_read_one_channel(int channel, int *row_val, int *process_val)
{
	int index = 0;
	int val = 0;
	struct iio_chan_spec local_chan = {0};
	if (g_indio_dev == NULL || channel > 15 || channel <0)
		return -1;
	local_chan.type = IIO_VOLTAGE;
	local_chan.indexed = 1;
	local_chan.info_mask_separate = BIT(IIO_CHAN_INFO_RAW) | BIT(IIO_CHAN_INFO_PROCESSED);
	for (index; index < ADC_CHANNEL_MAX; index++)
	{
		local_chan.channel = index;
		if (index == channel)
		{
			mt635x_auxadc_read_raw(g_indio_dev, &local_chan, row_val, &val, IIO_CHAN_INFO_RAW);
			mt635x_auxadc_read_raw(g_indio_dev, &local_chan, process_val, &val, IIO_CHAN_INFO_PROCESSED);
			pr_err("%s:success get auxadc value for channel[%d] raw_value %d process_val %d\n", __func__, index, *row_val, *process_val);
			return 0;
		}
	}
	pr_err("No such auxadc channel\n");
	return 1;
}
EXPORT_SYMBOL(mt6357_auxadc_read_one_channel);
...
extern u32 m10_batt_id;
static int auxadc_suspend_enter(void)
{
       if(m10_batt_id == 2){
                pr_info("%s batt id = %d,do not use mtk gauge ,skip\n",__func__,m10_batt_id);
                return 0;
        }
