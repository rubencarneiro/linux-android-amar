...
int tmp_bak = 250;
struct device *dev1;
bool disable_charger_by_factory = false;
static struct battery_data *batt;
extern int mt6357_auxadc_read_one_channel(int channel, int *row_val, int *process_val);
...
int fgauge_get_profile_id(void)
{
	...
	struct iio_channel *test_channel;int ret,val;
	test_channel = iio_channel_get(dev1,"batteryID-channel");
    ret = iio_read_channel_raw(test_channel,&val);
    printk("ID_ADC_channel=%d   ret=%d \n",val,ret);
	real_val = (val*1500)/4096;


	printk("[%s]BATTERY_ID_val = %d\n", __func__, real_val);

	if ((sizeof(g_battery_id_voltage) /
		sizeof(int)) != TOTAL_BATTERY_NUMBER) {
		printk("[%s]error! voltage range incorrect!\n",
			__func__);
		return 0;
	}

	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		if (real_val < g_battery_id_voltage[id]) {
			m10_batt_id = id;
			break;
		} else if (g_battery_id_voltage[id] == -1) {
			m10_batt_id = TOTAL_BATTERY_NUMBER - 1;
		}
	}


	printk("[%s]Battery id (%d)\n",
		__func__,
		m10_batt_id);
	return m10_batt_id;
}
...
static enum power_supply_property battery_props[] = {
	...
	POWER_SUPPLY_PROP_batt_info,
	POWER_SUPPLY_PROP_batt_vendor,
...

{
	switch (psp) {
		...
		printk("==============set POWER_SUPPLY_PROP_STATUS==========\r\n");
			if(val->intval == 1)
			{
				disable_charger_by_factory = true;
			}
			else if(val->intval == 0)
			{
				disable_charger_by_factory = false;
			}
			battery_update(batt->psy_cfg.drv_data);
			break;
		default:
			break;
	}
	...
	union power_supply_propval *val)
{
		...
		if(gm->battery_id == 2)
		{
			if (ext_psy) {
				...
				if(reta.intval < 0)
				{
					...
					printk("==========mm8013_disconnect================\r");
				}
				else
				{
					//printk("=========get value from mm8013================\r");
					val->intval = reta.intval;
				}
			}
		}
		if((val->intval == 100)&&(bs_data->bat_status == 1))
		{
			val->intval = 4;
		}
		else
	...
	case POWER_SUPPLY_PROP_batt_info:
	case POWER_SUPPLY_PROP_batt_vendor:
		val->intval = gm->battery_id;
		break;

	case POWER_SUPPLY_PROP_USB_OTG:
		ret = charger_dev_is_otg(charger_dev);
		val->intval = ret;
		break;
