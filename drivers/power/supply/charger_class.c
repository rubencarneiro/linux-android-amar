...
int charger_dev_is_otg(struct charger_device *chg_dev)
{
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->is_otg)
		return chg_dev->ops->is_otg();

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_is_otg);

int charger_dev_is_pogo_enable(struct charger_device *chg_dev)
{
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->is_otg)
		return chg_dev->ops->is_pogo_enable();

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_is_pogo_enable);

int charger_dev_pogo_enable(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->pogo_enable)
		return chg_dev->ops->pogo_enable(chg_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_pogo_enable);

int charger_dev_factory_discharge(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->factory_discharge)
		return chg_dev->ops->factory_discharge(chg_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_factory_discharge);
