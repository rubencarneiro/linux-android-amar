...
bool disable_charger_flag = false;
...
//#endif
	...
	prop.intval = en;
	if (en) {
		printk(" mtk_pmic_enable_chr_type_det   en true \n");
	    ...
	    printk(" mtk_pmic_enable_chr_type_det  en false charger type: charger OUT\n");
		ret = dts_bootmode;
		if (ret == KERNEL_POWER_OFF_CHARGING_BOOT||ret == LOW_POWER_OFF_CHARGING_BOOT)
		{
			mdelay(200);
			if(dock_chargered())
			{
				printk("====leon=====power off=====charger type DOCK========chgdet return========\r\n");
				return;
			}
		}
			...
			const union power_supply_propval *val)
{
	struct mtk_charger_type *info;
	union power_supply_propval reta = {0,};
	struct power_supply *ext_psy = power_supply_get_by_name("battery");
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		break;
	case POWER_SUPPLY_PROP_SHIP_MODE:
			printk("====================SHIP_MODE_FLAG_WRITE======================\r\n");
			charger_dev_enable_ship_mode(charger_dev,true);
			return 0;
	case POWER_SUPPLY_PROP_CHARGE_ENABLED:
		if(val->intval == 0)
		{
			charger_dev_factory_discharge(charger_dev,true);
			disable_charger_flag = true;
			reta.intval = 1;
		}
		else if(val->intval == 1)
		{
			disable_charger_flag = false;
			reta.intval = 0;
			charger_dev_factory_discharge(charger_dev,false);
		}
		power_supply_set_property(ext_psy,POWER_SUPPLY_PROP_STATUS, &reta);
		return 0;
	default:
		return -EINVAL;
	}
