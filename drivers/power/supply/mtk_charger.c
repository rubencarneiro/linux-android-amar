...
u32 dts_bootmode = 0;
static int supply_value;
static int charge_value;
...
void do_sw_jeita_state_machine(struct mtk_charger *info)
{
	...
	if(info->battery_temp_test != 99)
	{
		battery_temp = info->battery_temp_test;
	}
	else
	{
	...
	if (battery_temp >= info->data.temp_t4_thres) {
		chr_err("[SW_JEITA] Battery Over high Temperature(%d) !!\n",
			info->data.temp_t4_thres);

		sw_jeita->sm = TEMP_ABOVE_T4;
		sw_jeita->charging = false;
	} else if (battery_temp > info->data.temp_t3_thres) {
	...
	} else if (battery_temp >= info->data.temp_t2_thres) {
	...
	} else if (battery_temp > info->data.temp_t0_thres) {
		if ((sw_jeita->sm == TEMP_BELOW_T0)
		    && (battery_temp
			<= info->data.temp_t0_thres_plus_x_degree+1)) {
			chr_err("[SW_JEITA] Battery Temperature between %d and %d,not allow charging yet!!\n",
...
/*	if (sw_jeita->sm != TEMP_T2_TO_T3) {
	...
	printk("====================================[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d\n",
						...
						const char *buf, size_t size)
{
	...
	if (kstrtoint(buf, 10, &temp) == 0) {
		...
		chr_err("sw_jeita_store: format error!\n");
	}
	return size;
}
...
static ssize_t show_temp_jeita(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;

	printk("========================show_temp_jeita: %d\n", pinfo->battery_temp_test);
	return sprintf(buf, "%d\n", pinfo->battery_temp_test);
}

static ssize_t store_temp_jeita(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t size)
{
	struct mtk_charger *pinfo = dev->driver_data;
	signed int temp;
	...
	if (kstrtoint(buf, 10, &temp) == 0) {
		pinfo->battery_temp_test = temp;
		printk("========================store_temp_jeita: %d\n", pinfo->battery_temp_test);
	} else {
...
static DEVICE_ATTR(temp_test, 0664, show_temp_jeita,
		   store_temp_jeita);
...
static ssize_t supply_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	//struct mtk_charger *pinfo = dev->driver_data;

	chr_err("%s: %d\n", __func__, supply_value);

	return sprintf(buf, "%u\n", supply_value);
}
static ssize_t supply_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{	
	struct mtk_charger *pinfo = dev->driver_data;
	int ret = 0;
	...
	if (value ) {
		...
		charger_dev_set_charging_current(pinfo->chg1_dev,2000000);
		supply_value = 0;
...
static DEVICE_ATTR(supply_enable, 0664, supply_enable_show, supply_enable_store);
...
static void mtk_battery_notify_VBatTemp_check(struct mtk_charger *info)
{
	...
	if (battery_temp >= info->thermal.max_charge_temp) {
...
//	} else {
		...
		if (battery_temp <= info->thermal.min_charge_temp) {
			info->notify_code |= CHG_BAT_LT_STATUS;
			chr_err("bat_temp(%d) out of range(too low)\n",
				info->battery_temp);
			mtk_chgstat_notify(info);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (ret < 0) {
		 ...
		 mtk_battery_notify_check(pinfo);
		 _mtk_enable_charging(pinfo, value);
                 charge_value=value;
		 chr_err("%s: charging enble11 = %d value = %d \n", __func__, pinfo->can_charging, value);
	}
	return size;
}

static DEVICE_ATTR(charging_enable, 0664, charging_enable_show, charging_enable_store);
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	struct mtk_charger *info = platform_get_drvdata(pdev);
        ...
        ret = device_create_file(&(pdev->dev), &dev_attr_temp_test);
        if (ret)
                goto _out;
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	mtk_charger_parse_dt(info, &pdev->dev);
        chr_err("%s: device_create start \n", __func__);
        device_create_file(&pdev->dev, &dev_attr_charging_enable);
	device_create_file(&pdev->dev, &dev_attr_supply_enable);
        chr_err("%s: device_create start endstarts\n", __func__);
