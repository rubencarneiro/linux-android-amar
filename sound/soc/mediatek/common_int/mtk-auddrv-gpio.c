...
enum audio_system_gpio_type {
	...
	GPIO_RCVSPK_LOW,
#endif
	/*DSPG M10 audio bring up begin */

	GPIO_DBMDX_I2S_ON,
	GPIO_DBMDX_I2S_AC107_ON,
	GPIO_DBMDX_I2S_OFF,
	/*DSPG M10 end */
	GPIO_HPDEPOP_HIGH,
	GPIO_HPDEPOP_LOW,
...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
		...
		[GPIO_SMARTPA_AC107_ON] = {"aud_smartpa_ac107_on", false, NULL},
		...
		[GPIO_DBMDX_I2S_AC107_ON] = {"aud_dbmdx_ac107_on", false, NULL},
...
int AudDrv_GPIO_SMARTPA_Select(int mode)
{
	...
	case 1:
		...
		if (AudDrv_GPIO_IsValid(GPIO_SMARTPA_AC107_ON) && (ac107_chip_id == 0x4B))
			retval = AudDrv_GPIO_Select(GPIO_SMARTPA_AC107_ON);
		break;
	default:
...
int AudDrv_GPIO_EXTAMP_Select(int bEnable, int mode)
{
	...
	if (bEnable == 1)
	{
		if (mode == 1)
		{
			gpio_set_value(extamp_1,0);
			udelay(1);
			gpio_set_value(extamp_1,1);
		}
		else if (mode == 2)
		{
			gpio_set_value(extamp_1,0);
...
int Dbmdx_i2s_Control(int bEnable)
{
	...
	if(bEnable==1){
		...
		if (AudDrv_GPIO_IsValid(GPIO_DBMDX_I2S_AC107_ON) && (ac107_chip_id == 0x4B)){
			retval = AudDrv_GPIO_Select(GPIO_DBMDX_I2S_AC107_ON);
			printk(" dbmdx ac107 i2s enable\n");
		}
   }else{
...
int AudDrv_GPIO_EXTAMP2_Select(int bEnable, int mode)
{
	...
	if (bEnable == 1)
	{
		if (mode == 1)
		{
			gpio_set_value(extamp_2,0);
			udelay(1);
			gpio_set_value(extamp_2,1);
		}
		else if (mode == 2)
		{
			gpio_set_value(extamp_2,0);
...
static int audio_pa_probe(struct platform_device *pdev)
{
	...
	extamp_1 = of_get_named_gpio(node, "extamp_1", 0);
	if (extamp_1 < 0)
	{
		printk( "=============extamp_1 is not available\n");
		return -1;
	}

	extamp_2 = of_get_named_gpio(node, "extamp_2", 0);
	if (extamp_2 < 0)
	{
		printk( "=============extamp_2 is not available\n");
		return -1;
	}

	gpio_request(extamp_1,"extamp_1");
	gpio_direction_output(extamp_1,0);


	gpio_request(extamp_2,"extamp_2");
	gpio_direction_output(extamp_2,0);


		printk( "============audio_pa_probe_ok\n");

    return 0;
}

static struct of_device_id audio_pa_table[] = {
...
static struct platform_driver audio_pa_driver = {
	...
	.driver = {
		.name = "audio_pa",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(audio_pa_table),
	},
};
...
static int __init audio_drv_gpio_init(void)
{
	...
	platform_driver_register(&audio_pa_driver);
	return 0;
}
