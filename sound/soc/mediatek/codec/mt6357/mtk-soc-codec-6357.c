...
/*DSPG M10 not allow disable micbias after stop record,call begin */
bool dspg_micbias_status=true;
static bool mtk_micbias_status=true;
/*DSPG M10 end */
...
static void Ext_Speaker_Amp1_Change(bool enable)
{
	pr_debug("%s(), enable %d\n", __func__, enable);
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
...
static void Ext_Speaker_Amp2_Change(bool enable)
{
	pr_debug("%s(), enable %d\n", __func__, enable);
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
...
#ifdef CONFIG_AC107
static int Ac107_Get(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
        ...
        */
       ucontrol->value.integer.value[0] =mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_AC107];
       return 0; 
}

extern void ac107_start(void);
extern void ac107_stop(void);

static int Ac107_Set(struct snd_kcontrol *kcontrol,
                               struct snd_ctl_elem_value *ucontrol)
{
       pr_info("%s()\n", __func__);
       mutex_lock(&Ana_Power_Mutex);
       if (ucontrol->value.integer.value[0]) {
		ac107_start();
               mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_AC107] =ucontrol->value.integer.value[0];
       } else {
		ac107_stop();
               mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_AC107] =ucontrol->value.integer.value[0];
       }
...
static int Micbias0_On(bool on)
{
       pr_info("%s() on:%d\n", __func__, on);
       mutex_lock(&Ana_Power_Mutex);
       if( on){
...
static int Speaker_Left_Pa_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_err("%s()  spk pa = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0]) {
		Ext_Speaker_Amp1_Change(true);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_SPK_LEFT_PA] =
		    ucontrol->value.integer.value[0];
	} else {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_SPK_LEFT_PA] =
		    ucontrol->value.integer.value[0];
		Ext_Speaker_Amp1_Change(false);
	}
	return 0;
}
...
static int Speaker_Right_Pa_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_err("%s()  spk pa = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0]) {
		Ext_Speaker_Amp2_Change(true);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_SPK_RIGHT_PA] =
		    ucontrol->value.integer.value[0];
	} else {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_SPK_RIGHT_PA] =
		    ucontrol->value.integer.value[0];
		Ext_Speaker_Amp2_Change(false);
	}
	return 0;
}
...
static const struct snd_kcontrol_new mt6357_snd_controls[] = {
	...
	SOC_ENUM_EXT("Left_Pa_Switch", Audio_DL_Enum[16],
		     Speaker_Left_Pa_Get,
		     Speaker_Left_Pa_Set),
	SOC_ENUM_EXT("Right_Pa_Switch", Audio_DL_Enum[17],
		     Speaker_Right_Pa_Get,
		     Speaker_Right_Pa_Set),
        SOC_ENUM_EXT("Micbias0_Switch", Audio_DL_Enum[15],
             Micbias0_Get,
             Micbias0_Set),
            #ifdef CONFIG_AC107
        SOC_ENUM_EXT("Ac107_Switch", Audio_DL_Enum[18],
             Ac107_Get,
             Ac107_Set),
            #endif
};
	...
	if (enable) {
		if (GetAdcStatus() == false) {
			...
			if (mCodec_data->mAudio_Ana_Mux
				...
				/*DSPG M10 call not allow disable micbias after dspg not use */
		...
		if (GetAdcStatus() == false) {
			...
			if (mCodec_data->mAudio_Ana_Mux
				...
				/* phone mic */
				/* Disable MICBIAS0, MISBIAS0 = 1P7V */
				/*DSPG M10 not allow disable micbias after stop record,call */
				#if 1
