...
#define DRIVER_VERSION				"4.047.5.20210127h"
...
static int dbmdx_va_ve_set_asrp_gain(struct dbmdx_private *p,
					struct usecase_config *usecase,
					bool mute)
{
	...
	gain_type = usecase->asrp_output_gain_type;

	if (!mute) {
		if (p->va_ve_gain_mode == 1) {
			asrp_tx_out_gain =  (u16)(p->pdata->asrp_tx_out_gain);
			asrp_vcpf_out_gain =
					(u16)(p->pdata->asrp_vcpf_out_gain);
			asrp_rx_out_gain = (u16)(p->pdata->asrp_rx_out_gain);
		} else {
			asrp_tx_out_gain =  (u16)(usecase->asrp_tx_out_gain);
			asrp_vcpf_out_gain = (u16)(usecase->asrp_vcpf_out_gain);
			asrp_rx_out_gain = (u16)(usecase->asrp_rx_out_gain);
		}
	}
...
static int dbmdx_mute_current_usecase(struct dbmdx_private *p, bool mute)
{
	...
	ret = dbmdx_va_ve_set_asrp_gain(p, cur_usecase, mute);
					...
					enum dbmdx_chip chip)
{
	...
	} else if (chip == DBMDX_CHIP_VA_VE && p->va_ve_chip_enabled &&
...
static int dbmdx_check_asrp_params_loaded_correctly(struct dbmdx_private *p)
{
	...
	ret = dbmdx_send_cmd(p, DBMDX_REGN_ASRP_LIB_VER, &val);
	if (ret < 0) {
		dev_err(p->dev,
			"%s: failed to load ASRP\n",
			__func__);
		return -EIO;
	}

	dev_info(p->dev, "%s: ASRP version: 0x%x\n", __func__, val);
						...
						struct usecase_config *uc)
{
	...
	for (i = 0; i < DBMDX_MAX_SUPPORTED_VT_ENGINES; i++) {
		...
		if (vte->enabled &&
			...
			dev_info(p->dev,
				"%s: Detection for VTE (%s) was enabled\n",
				__func__,
				dbmdx_vte_names[vte->eng_type]);
			vte->internal_state = MODEL_STATE_STARTED;
			...
			if (vte->enabled) {
				...
				dev_info(p->dev,
				"	%s: Detection for VTE (%s) was disabled\n",
					__func__,
					dbmdx_vte_names[vte->eng_type]);
			}
		}
...
{
#ifndef   USE_KERNEL_ABOVE_4_17_XX
