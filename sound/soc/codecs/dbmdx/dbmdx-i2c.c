	...
	u32 command, u16 *response)
{
	...
	usleep_range(DBMDX_USLEEP_I2C_VQE_CMD_AFTER_SEND + p->debug_delay_usec,
		DBMDX_USLEEP_I2C_VQE_CMD_AFTER_SEND +
					p->debug_delay_usec + 1000);
				  ...
				  u16 *response)
{
	...
	if (response) {
		...
		usleep_range(DBMDX_USLEEP_I2C_VA_CMD_AFTER_SEND +
							p->debug_delay_usec,
			DBMDX_USLEEP_I2C_VA_CMD_AFTER_SEND +
						p->debug_delay_usec + 1000);
	...
	usleep_range(DBMDX_USLEEP_I2C_VA_CMD_AFTER_SEND_2 + p->debug_delay_usec,
		DBMDX_USLEEP_I2C_VA_CMD_AFTER_SEND_2 +
						p->debug_delay_usec + 1000);
		...
		size_t *data_offset)
{
	...
	ret = send_i2c_cmd_va(p, DBMDX_REGN_READ_AUDIO_BUFFER | samples, NULL);
