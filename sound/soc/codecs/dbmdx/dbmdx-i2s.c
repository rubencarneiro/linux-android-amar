...
#ifdef SND_SOC_COMPONENT
		struct snd_soc_component *component = dai->component;
		dev_dbg(component->dev, "%s: dai:%d tristate:%d\n", __func__,
			dai->id, tristate);
#else 
		struct snd_soc_codec *codec = dai->codec;
...
#ifdef SND_SOC_COMPONENT
		struct snd_soc_component *component = dai->component;
		dev_dbg(component->dev, "%s: dai:%d mute:%d\n", __func__,
			dai->id, mute);
#else 
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		#ifdef SND_SOC_COMPONENT    
		dev_dbg(component->dev, "%s: playback stream\n", __func__);
		#else        
		dev_dbg(codec->dev, "%s: playback stream\n", __func__);
		#endif
	else
		#ifdef SND_SOC_COMPONENT
        dev_dbg(component->dev, "%s: capture stream\n", __func__);     
		#else
			       ...
			       struct snd_soc_dai *dai)
{
		...
		dev_dbg(component->dev, "%s: params_channels = %d\n", __func__,
...
#ifdef SND_SOC_COMPONENT 
	dev_dbg(component->dev, "%s: params_rate = %d\n", __func__,
			params_rate(params));
#else
			     ...
			     struct snd_soc_dai *dai)
{
		...
		dev_dbg(component->dev, "%s: dai:%d cmd=%d\n", __func__,
			dai->id, cmd);
