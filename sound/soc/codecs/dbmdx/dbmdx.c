...
/* Global Variables */
static int mic_init_flag=1;
static int mic_opened_first=1;
							...
							bool allow_sleep)
{
	...
	/*mic initialization in the first time to enter bargin mode*/
	//if(mic_init_flag)
	{
...
#endif
{
	...
	if (p->pdata->feature_vqe) {
#ifdef SND_SOC_COMPONENT
		rc = snd_soc_add_component_controls(component, dbmdx_vqe_snd_controls,
				ARRAY_SIZE(dbmdx_vqe_snd_controls));
		...
		struct dbmdx_private *p)
{
	...
	if (ret && ret != -EINVAL) {
		...
		if(ac107_chip_id == 0){
			p->pdata->asrp_delay = 30;
		}else{
			p->pdata->asrp_delay = 1;
		}
	}
