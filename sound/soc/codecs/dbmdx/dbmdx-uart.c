...
#define DBMDX_UART_READ_REG_COMMAND_LEN	4
#define DBMDX_UART_WRITE_32_COMMAND_LEN	12
#define DBMDX_UART_WRITE_32_RETURN_LEN	9

ssize_t send_uart_cmd_write_va_32(struct dbmdx_private *p, u16 register_number,
								 u32 value)
{
	struct dbmdx_uart_private *uart_p =
				(struct dbmdx_uart_private *)p->chip->pdata;

	u8 send[DBMDX_UART_WRITE_32_COMMAND_LEN + 1];
	int ret = 0;
	...
	ret = snprintf(send, (DBMDX_UART_WRITE_32_COMMAND_LEN + 1), "%03xW%08x",
				register_number, value);

	dev_dbg(uart_p->dev, "%s: Send %s\n", __func__, send);

	if (ret >= 0) {
		ret = uart_write_data(p, send, DBMDX_UART_WRITE_32_COMMAND_LEN);
	} else {
		ret = -EIO;
		goto out;
	}

	if (ret != DBMDX_UART_WRITE_32_COMMAND_LEN) {
		ret = -EIO;
		goto out;
	}
...
out:
	usleep_range(DBMDX_USLEEP_UART_VA_CMD_AFTER_SEND_2 +
					p->debug_delay_usec,
		DBMDX_USLEEP_UART_VA_CMD_AFTER_SEND_2 +
...
ssize_t send_uart_cmd_read_va_32(struct dbmdx_private *p, u16 register_number,
	u32 *response)
{
	struct dbmdx_uart_private *uart_p =
				(struct dbmdx_uart_private *)p->chip->pdata;
	u8 recv[DBMDX_UART_WRITE_32_RETURN_LEN + 1];
	u8 send[DBMDX_UART_READ_REG_COMMAND_LEN + 1];
	int ret;

	memset(recv, 0, (DBMDX_UART_WRITE_32_RETURN_LEN + 1));
	...
	ret = snprintf(send, (DBMDX_UART_READ_REG_COMMAND_LEN + 1),
				"%03xR", register_number);

	dev_dbg(uart_p->dev, "%s: Send %s\n", __func__, send);
	if (ret >= 0) {
		ret = uart_write_data(p, send, DBMDX_UART_READ_REG_COMMAND_LEN);
	} else {
		ret = -EIO;
		goto out;
	}

	if (ret != DBMDX_UART_READ_REG_COMMAND_LEN) {
		ret = -EIO;
		goto out;
	}

	usleep_range(DBMDX_USLEEP_UART_VA_CMD_AFTER_SEND + p->debug_delay_usec,
		DBMDX_USLEEP_UART_VA_CMD_AFTER_SEND +
					p->debug_delay_usec + 100);
	...
	ret = uart_read_data(p, recv, DBMDX_UART_WRITE_32_RETURN_LEN);
	if (ret < 0) {
		dev_err(uart_p->dev, "%s:uart_read failed =%d\n",
			__func__, ret);
		return ret;
	}
	recv[DBMDX_UART_WRITE_32_RETURN_LEN] = 0;
	ret = kstrtou32((const char *)&recv, 16, response);
	if (ret < 0) {
		dev_err(uart_p->dev, "%s failed -%d\n", __func__, ret);
		dev_err(uart_p->dev, "%s: %x:%x:%x:%x: %x:%x:%x:%x:\n",
	...
	dev_dbg(uart_p->dev, "%s: Received 0x%08x\n", __func__, *response);
...
ssize_t send_uart_cmd_va_32(struct dbmdx_private *p, u32 command,
				   u32 value, u32 *response)
{
	...
	if (response)
		ret = send_uart_cmd_read_va_32(p, register_number, response);
	else
		ret = send_uart_cmd_write_va_32(p, register_number, value);
				   ...
				   u16 *response)
{
	...
	if (response) {
		...
		usleep_range(DBMDX_USLEEP_UART_VA_CMD_AFTER_SEND +
							p->debug_delay_usec,
			DBMDX_USLEEP_UART_VA_CMD_AFTER_SEND_2 +
						p->debug_delay_usec + 100);
...
int uart_wait_till_alive(struct dbmdx_private *p)
{
	...
	do {
		...
		ret = send_uart_cmd_va(p, DBMDX_REGN_CHIP_ID_NUMBER, &response);
		if (ret)
			continue;
	...
	size_t *data_offset)
{
	...
	ret = send_uart_cmd_va(p, DBMDX_REGN_READ_AUDIO_BUFFER | samples, NULL);
...
int uart_common_probe(struct platform_device *pdev, const char threadnamefmt[])
{
...
#ifdef CONFIG_PM_WAKELOCKS
	p->ps_nosuspend_wl = wakeup_source_register(p->dev, "dbmdx_nosuspend_wakelock_uart");
	...
	p->chip.send_cmd_va_32 = send_uart_cmd_va_32;
