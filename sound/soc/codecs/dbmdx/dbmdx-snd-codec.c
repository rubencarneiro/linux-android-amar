...
static int pcm_command_in_progress(struct snd_dbmdx_runtime_data *prtd,
...
static void wait_for_pcm_commands(struct snd_dbmdx_runtime_data *prtd)
{
	int ret;
...
int check_dbmdx_status(void){
return dbmdx_not_ready;
}

EXPORT_SYMBOL(check_dbmdx_status);
...
int dbmdx_snd_init(struct dbmdx_private *p)
{
	...
	if (!prtd->dbmdx_pcm_workq) {
		...
		return snd_soc_register_component(p->dev, &soc_component_dev_dbmdx,
				&dbmdx_codec_dai, 1);
#else
